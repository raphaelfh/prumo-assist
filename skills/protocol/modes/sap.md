...
prumo:
  ...
  requires: [cli]
  phrases:
    - "gera o plano de análise estatística"
    - "justifica o tamanho amostral"
    - "planeja as análises de sensibilidade"
  legacy: [write-statistics]
  write_kind: statistics
  disclosure_task: "drafting of the statistical analysis plan"
---
...
co-localizado: [`../templates/sap.md`](../templates/sap.md). Override por projeto:
...
5. **Citação strict**, idêntica ao `write manuscript`.

## Fluxo

Mesmo fluxo do `write manuscript`, com `--kind statistics` (template = `../templates/sap.md`):
