...
| **Groundedness** — `paper support` julga se a fonte sustenta a frase lendo `_extract.md`, artefato de LLM, nunca o PDF | `skills/paper/modes/support.md` passo 3; `docs/references/pdfs/<citekey>.pdf` é caminho construível e `Read` lê PDF nativo (ADR-0013). Extract errado produz confirmação ativa, não silêncio | **atingido** — a skill está publicada |
