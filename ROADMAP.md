...
| **Proveniência ligada** — Princípio V está escrito e ligado em um produtor só | `paper extract` carimba `_meta` via `build_meta` desde a F2 da superfície de skills; `TraceWriter` e os demais produtores (findings, write, export) seguem sem `_meta`, e `domains/write/disclosure.py` ainda lê os sinais heterogêneos | **atingido** — dívida parcial, declarada no código |
