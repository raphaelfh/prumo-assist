...
| ~~**Proveniência ligada** — Princípio V está escrito e ligado em um produtor só~~ **resolvido** | Findings, `wiki study`, `write draft` e `citemap.json` carimbam `_meta`; disclosure lê o canônico; `TraceWriter` sem uso removido ([ADR-0035](docs/adr/adr-0035-meta-embutido-sem-trace.md)) | — |
| ~~**Figuras e tabelas** — sem endereço, sem numeração, sem proveniência~~ **resolvido** | Filtro Lua vendorizado numera figuras e tabelas e resolve `@fig:`/`@tbl:` no export ([ADR-0034](docs/adr/adr-0034-figuras-e-tabelas-por-filtro-lua.md)); proveniência de figura segue sem gatilho | — |
